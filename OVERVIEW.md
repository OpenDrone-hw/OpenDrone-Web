...
flowchart LR
    ...
    STK["stock/product_skus.json"]
    CMP["compliance/webshop/<br/>NL legal markdown"]
    BR["OpenDrone/brand/ · onshape/<br/>logos · tokens · STEP"]
    WEB["OpenDrone/web<br/>Hydrogen storefront · opendrone.be"]
    SHOP["SHOPIFY<br/>Storefront · Customer Account<br/>Admin API · Oxygen"]
    GH["GITHUB<br/>topics · releases · contributors"]
    DSC["DISCORD<br/>support forum threads"]

    HW -->|"board art · schematics<br/>specs · downloads"| WEB
    STK -->|"sync-product-skus.mjs"| WEB
    CMP -->|"sync-legal.mjs at prebuild"| WEB
    BR -.->|"gen-site-assets · build-hero"| WEB
    GH -->|"status → roadmap<br/>timeline · votes"| WEB
    WEB <-->|"products · cart · customers"| SHOP
    WEB -->|"hydrogen deploy on every push"| SHOP
...
flowchart TB
    ...
    subgraph RT["RUNTIME CONTRACT"]
        ...
        R5["docs/<br/>maintainer notes · not routed"]
    end
    ...
    subgraph CI[".github/workflows/"]
        ...
        W3["support-notify · support-cleanup<br/>timeline-ledger · community-sync"]
    end
...
```mermaid
%%{init: {"theme": "base", "themeVariables": {"background": "#0b1120", "lineColor": "#cbd5e1", "primaryTextColor": "#f8fafc", "edgeLabelBackground": "#0b1120", "fontSize": "14px"}, "flowchart": {"curve": "linear", "nodeSpacing": 26, "rankSpacing": 50}}}%%
flowchart LR
...
```mermaid
%%{init: {"theme": "base", "themeVariables": {"background": "#0b1120", "lineColor": "#cbd5e1", "primaryTextColor": "#f8fafc", "edgeLabelBackground": "#0b1120", "fontSize": "14px"}, "flowchart": {"curve": "linear", "nodeSpacing": 26, "rankSpacing": 60}}}%%
flowchart LR
    S1["hardware · .kicad_pcb"]
    S2["hardware · README ## Specifications"]
    S3["hardware · GitHub release assets"]
    S4["hardware · status-* topic"]
    SKU["stock/product_skus.json"]
    LEG["compliance/webshop/*.md"]
    ...
    SKU -->|"sync-product-skus.mjs<br/>match handle + Model option"| O6["Shopify variant SKU"]
    LEG -->|"sync-legal.mjs · prebuild"| O7["app/content/legal/nl/"]
...
flowchart TB
    subgraph EDIT["EDIT · local"]
        direction LR
        DEV["npm run dev<br/>shopify hydrogen dev --codegen"]
        STU["/studio<br/>dev-only panels"]
        FILES["content/copy · content/products<br/>content/theme.json · app/content/legal/**<br/>public/models/&lt;design&gt;/studio.json"]
        DIFF["git diff = changelog"]
        DEV --> STU -->|"vite-plugin-studio write endpoint"| FILES --> DIFF
    end

    subgraph SHIP["SHIP · GitHub"]
        direction LR
        PR["PR · ci.yml<br/>lint · typecheck · test<br/>registry · status · build"]
        MAIN["main"]
        OXY["Oxygen · opendrone.be"]
        PR --> MAIN -->|"oxygen-deployment workflow<br/>every push"| OXY
    end

    EDIT -->|"npm run build locally first<br/>prebuild sync:legal → hydrogen build"| SHIP
    ...
    style EDIT fill:transparent,stroke:#475569,color:#cbd5e1;
    style SHIP fill:transparent,stroke:#0f766e,color:#99f6e4;
```
